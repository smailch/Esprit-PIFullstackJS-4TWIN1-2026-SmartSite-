...
export function DreamHouseClient() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-slate-900/45 px-4 py-3 backdrop-blur">
        ...
        <p className="text-xs text-slate-300">Workspace: Dream House generator</p>
      </div>

      <header className="mb-8 rounded-3xl border border-white/10 bg-slate-900/45 p-6 shadow-lg shadow-black/20 backdrop-blur sm:p-8">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-orange-400">Visualization</p>
        <h1 className="mt-2 text-balance text-3xl font-semibold tracking-tight text-slate-50 sm:text-4xl">
          Dream House
        </h1>
        <p className="mt-3 max-w-3xl text-pretty text-sm leading-relaxed text-slate-300 sm:text-base">
          Build a strong prompt, generate multiple facade views, then review the 3D model once Tripo finishes.
          This page is organized as a two-step workflow to keep generation and review clear.
        </p>
        <div className="mt-5 grid gap-3 text-xs sm:grid-cols-3">
          <div className="rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2 text-slate-300">
            <span className="font-semibold text-slate-100">Step 1</span>: Define style, details, budget, and color.
          </div>
          <div className="rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2 text-slate-300">
            <span className="font-semibold text-slate-100">Step 2</span>: Inspect generated image views.
          </div>
          <div className="rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2 text-slate-300">
            <span className="font-semibold text-slate-100">Step 3</span>: Validate and navigate the 3D model.
          </div>
        </div>
      </header>

      <div className="grid gap-8 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <section className="rounded-3xl border border-white/10 bg-slate-900/40 p-6 shadow-lg shadow-black/20 backdrop-blur-sm sm:p-8">
          <div className="mb-5 flex items-center justify-between gap-3">
            <h2 className="text-lg font-semibold text-slate-100">Project brief</h2>
            <span className="rounded-full border border-white/15 bg-slate-950/70 px-3 py-1 text-[11px] font-medium uppercase tracking-wide text-slate-300">
              Input panel
            </span>
          </div>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5">
              ...
              <Button type="submit" size="lg" className="mt-2 w-full rounded-2xl sm:w-auto" disabled={formDisabled}>
                {starting ? (
                  <>
        ...
        <section className="min-w-0 space-y-6 xl:sticky xl:top-6 xl:self-start">
          {!showResults ? (
            <div className="rounded-3xl border border-dashed border-white/20 bg-slate-900/30 px-6 py-10 text-center">
              <p className="text-sm font-medium text-slate-200">No generation yet</p>
              <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-slate-400">
                Complete the brief and launch generation. Image previews will appear here first, then the 3D model
          ...
          {showResults ? (
            <>
              <div className="rounded-3xl border border-white/10 bg-slate-900/40 p-5 shadow-lg shadow-black/20">
                <h3 className="mb-1 text-sm font-semibold uppercase tracking-wide text-slate-400">
                Image previews
                </h3>
                <p className="mb-3 text-xs leading-relaxed text-slate-400">
                  Generated illustrations (Pollinations) for inspiration. Fetching is staggered by{" "}
                  {Math.round(POLLINATIONS_PROXY_GAP_MS / 1000)} s between views to reduce throttling.
                </p>
                ...
                {imageUrls.length > 1 ? (
                  ...
                  <div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-900/50 shadow-inner" ref={emblaRef}>
                    <div className="flex">
                    {imageUrls.map((url, i) => (
                      <div
                      ...
                      >
                        <div className="relative aspect-video w-full">
                          {slideErrors[i] ? (
                            ...
                            // eslint-disable-next-line @next/next/no-img-element -- blob: local after backend proxy
                            <img
                ...
                {imageUrls.length > 1 ? (
                  ...
                  {imageUrls.map((url, i) => (
                    ...
                    >
                      {proxiedImageUrls[i] ? (
                        // eslint-disable-next-line @next/next/no-img-element -- blob thumbnail
                        <img
                          src={proxiedImageUrls[i]}
                          alt=""
              ...
              <div className="rounded-3xl border border-white/10 bg-slate-900/40 p-5 shadow-lg shadow-black/20">
                ...
                <p className="mb-3 text-xs leading-relaxed text-slate-400">
                  Tripo builds the model from the main view (or text fallback). Backend hints:{" "}
                  <code className="rounded bg-slate-800 px-1 py-0.5 text-[11px]">TRIPO_FACE_LIMIT</code>,{" "}
                  <code className="rounded bg-slate-800 px-1 py-0.5 text-[11px]">TRIPO_TEXTURE</code>.
                </p>
              {tripoError ? (
                <p
                  role="alert"
