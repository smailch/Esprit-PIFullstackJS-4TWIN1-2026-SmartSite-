  ...
  const tableColumns: Column<ProjectTableRow>[] = [
    {
      key: 'name' as const,
      label: 'Project Name',
      headerClassName: 'min-w-0 w-[24%] lg:w-[22%] xl:w-[15%]',
      cellClassName: 'align-top',
      render: (value: string | number | undefined, row: ProjectTableRow) => (
        <div className="flex min-w-0 items-start gap-2 sm:gap-2.5">
          <span className="mt-0.5 shrink-0">{getProjectIcon(0)}</span>
          <div className="min-w-0 flex-1">
            <p className="line-clamp-2 font-semibold leading-snug text-foreground [overflow-wrap:break-word] [word-break:normal]">
              {isValidObjectId(row._id) ? (
                <Link
                  href={`/projects/${row._id}/overview`}
                  className="block rounded-sm hover:text-primary hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
            ...
            <p
              className="line-clamp-2 max-w-full text-sm text-muted-foreground [overflow-wrap:break-word] [word-break:normal]"
              title={row.description?.trim() ? row.description : undefined}
            >
    ...
    },
    {
      ...
      label: 'Description',
      headerClassName: 'hidden min-w-0 xl:table-cell xl:w-[9%]',
      cellClassName: 'hidden align-top xl:table-cell',
    ...
    },
    {
      key: 'type' as const,
      label: 'Type',
      headerClassName: 'min-w-0 w-[7%]',
      cellClassName:
        'align-top text-xs leading-snug sm:text-sm [overflow-wrap:break-word] [word-break:normal]',
      render: (value: string | number | undefined) =>
    ...
    },
    {
      ...
      headerClassName: 'min-w-0 w-[7%]',
      cellClassName:
        'align-top text-end text-[11px] leading-tight tabular-nums [overflow-wrap:anywhere] sm:text-xs',
      render: (value: number | string | undefined) =>
        value != null && value !== '' && !Number.isNaN(Number(value)) ? (
          <span
            className="inline-block max-w-full text-right font-medium text-foreground"
            title={
    ...
    },
    {
      ...
      label: 'Spent budget',
      align: 'right' as const,
      headerClassName: 'hidden min-w-0 lg:table-cell lg:w-[7%]',
      cellClassName:
        'hidden align-top text-end text-[11px] leading-tight tabular-nums [overflow-wrap:anywhere] sm:text-xs lg:table-cell',
      render: (_value: string | number | undefined, row: ProjectTableRow) => (
        <span
          ...
          title={formatDh(row.spentBudget)}
        >
          {new Intl.NumberFormat('fr-FR', {
            style: 'currency',
            currency: 'EUR',
            ...
            notation: 'compact',
            compactDisplay: 'short',
          }).format(Number(row.spentBudget ?? 0))}
        </span>
      ),
    },
    {
      ...
      label: 'Location',
      headerClassName: 'min-w-0 w-[10%] lg:w-[9%]',
      cellClassName: 'align-top',
      render: (value: string | number | undefined) => {
        const s = value != null ? String(value).trim() : '';
        return s ? (
          <span className="line-clamp-2 text-xs leading-snug sm:text-sm [overflow-wrap:break-word] [word-break:normal]">
            {s}
          </span>
        ) : (
          '—'
    ...
    },
    {
      ...
      label: 'Start Date',
      headerClassName: 'min-w-0 w-[8%] lg:w-[7%]',
      cellClassName: 'align-top whitespace-nowrap text-[10px] tabular-nums sm:text-[11px]',
      render: (val: any) =>
        val
          ? new Date(val).toLocaleDateString(undefined, { day: '2-digit', month: '2-digit', year: 'numeric' })
          : '—',
    },
    {
    ...
    },
    {
      key: 'status' as const,
      label: 'Status',
      headerClassName: 'min-w-0 w-[9%] lg:w-[8%]',
      cellClassName:
        'align-top text-center text-[10px] [overflow-wrap:break-word] [word-break:normal] sm:text-[11px]',
      render: (value: string | number | undefined) => (
        <div className="flex w-full min-w-0 justify-center">
          <span
            className={cn(
              getStatusColor(String(value ?? '')),
              '!inline-flex !max-w-full !justify-center !whitespace-normal !px-1.5 !py-0.5 !text-[10px] !leading-tight [overflow-wrap:break-word] [word-break:normal] sm:!px-2 sm:!text-[11px]',
            )}
            title={projectStatusLabel(String(value ?? ''))}
          >
    ...
    },
    {
      ...
      headerClassName: 'min-w-0 w-[27%] lg:w-[26%] xl:w-[24%]',
      cellClassName: 'align-top',
      render: (_value: string | number | undefined, row: ProjectTableRow) => (
        <div
          role="group"
          aria-label={`Actions for ${row.name}`}
          className="mx-auto flex max-w-full flex-wrap items-center justify-center gap-1 sm:gap-1.5"
        >
          <button
            type="button"
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center rounded-lg bg-emerald-800 text-white shadow-sm transition-[filter] hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-45 sm:size-9"
            disabled={!isValidObjectId(row._id)}
          >
            <Brain size={16} className="shrink-0 sm:h-[18px] sm:w-[18px]" aria-hidden />
          </button>
          <button
            type="button"
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center rounded-lg bg-teal-800 text-white shadow-sm transition-[filter] hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-45 sm:size-9"
            disabled={!isValidObjectId(row._id)}
          >
            <MessageCircle size={16} className="shrink-0 sm:h-[18px] sm:w-[18px]" aria-hidden />
          </button>
          <button
            type="button"
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center rounded-xl bg-orange-500 text-white shadow-sm transition-all duration-200 hover:bg-orange-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-45 sm:size-9"
            disabled={!isValidObjectId(row._id)}
          >
            <Sparkles size={16} className="shrink-0 sm:h-[18px] sm:w-[18px]" aria-hidden />
          </button>
          <button
            type="button"
            title="Edit"
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground shadow-sm transition-[filter] hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-45 sm:size-9"
            disabled={!isValidObjectId(row._id)}
          >
            <Pencil size={16} className="shrink-0 sm:h-[18px] sm:w-[18px]" aria-hidden />
          </button>
          <button
            type="button"
            title="Delete"
            ...
            className="inline-flex size-8 shrink-0 items-center justify-center rounded-lg border border-destructive/30 bg-background text-destructive shadow-sm transition-colors hover:bg-destructive/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-45 sm:size-9"
            disabled={
              !isValidObjectId(row._id) ||
              (isDeleteProjectSubmitting && projectPendingDelete?.id === row._id)
            }
          >
            <Trash2 size={16} className="shrink-0 sm:h-[18px] sm:w-[18px]" aria-hidden />
          </button>
          <button
            type="button"
            ...
            className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-border bg-secondary px-1.5 py-1.5 text-[11px] font-semibold text-foreground shadow-sm transition-colors hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 sm:gap-1.5 sm:px-2.5 sm:py-2 sm:text-xs md:px-3"
            disabled={!isValidObjectId(row._id)}
          >
            <LayoutDashboard size={14} className="shrink-0 text-primary sm:h-4 sm:w-4" aria-hidden />
            <span className="hidden min-[1180px]:inline">Overview</span>
          </button>
          <button
            type="button"
            ...
            disabled={!isValidObjectId(row._id)}
          >
            <BarChart3 size={14} className="shrink-0 text-primary sm:h-4 sm:w-4" aria-hidden />
            <span className="hidden min-[1180px]:inline">Gantt</span>
          </button>
        </div>
      ),
    },
