...
export class DreamHouseService {
  ...
  buildImagePrompt(
    ...
    const composed = `Photorealistic exterior, single-family house. ${shot}${view}${colorBit} ${brief} Daylight, architectural visualization, sharp detail.`;
  ...
  /** Prompt 3D pour Tripo — forme, couleur d’accent, description. */
  buildTripoPrompt(dto: DreamHouseRequestDto): string {
    const brief = dto.description.trim();
    const hex = dto.accentColor.trim();
    let p = `Residential house 3D model, clean geometry, coherent architecture, high detail; exterior accent color ${hex} on walls trim shutters or door where appropriate: ${brief}`;
