...
} from './attendance-monthly-prime.metrics';
import {
  attendanceBonusAnalysisPayloadSchema,
  attendanceBonusInsightsResponseSchema,
  type AttendanceBonusAnalysisPayload,
  type AttendanceBonusInsightsResponse,
  type TravailleurBonusRow,
} from './attendance-bonus.zod';
...
Exact structure:
{
  "summary": string (2–4 sentences on team dynamics for the month),
  "recommendationsEquipe": string[] (2–5 items),
  "travailleurs": [
    {
      "resourceId": string (Mongo ObjectId exactly as in the data),
      "nomAffiche": string,
      "scorePerformance": number between 0 and 30 (must align with server pointsMensuel),
      "montantPrimeSuggere": number | null (in DT, align with server primeDt when possible),
      "montantBonusSuggere": null (reserved, keep null),
      "justification": string (1–3 factual sentences),
      "pointsForts": string[] (1–4 items)
    }
  ],
  "confiance": number between 0 and 1
}

Rules:
- Do not contradict server-provided points and working days.
- Include **every** worker listed in backendMetricsParTravailleur.
- If data is sparse, lower confidence.` as const;
...
export class AnalysisAiService {
  ...
  /**
   * Pointage mensuel : points sur jours ouvrables (week-ends exclus), prime en DT (barème serveur), texte optionnel Groq.
   */
  async generateAttendanceBonusInsights(
    jobId: string,
    year: number,
    month: number,
    ...
    if (backendMetrics.length === 0) {
      throw new BadRequestException(
        `No attendance for this job in ${month}/${year}. Record attendance for this month.`,
      );
    }
    ...
    const payload = {
      ...
      backendMetricsParTravailleur: backendMetrics,
      nombreEnregistrements: attendance.length,
    };
    const contextJson = JSON.stringify(payload);

    let rawText: string;
    try {
      rawText = await this.callGroqJson(
        ATTENDANCE_BONUS_SYSTEM_PROMPT,
      ...
      return this.finalizeAttendanceBonusResponse(
        ...
        month,
        backendMetrics,
        this.buildFallbackAttendanceAnalysis(backendMetrics),
        'fallback',
        now,
      );
    }
  ...
  private static readonly REGLE_PRIME_DT =
    'Working days only. Scale: 30 pts → 50 DT ; 29 → 30 DT ; 28 → 10 DT.';

  private finalizeAttendanceBonusResponse(
    jobId: string,
    jobTitle: string,
    year: number,
    month: number,
    backendMetrics: WorkerMonthlyAttendanceMetrics[],
    analysis: AttendanceBonusAnalysisPayload,
    source: 'groq' | 'fallback',
    now: Date,
  ): AttendanceBonusInsightsResponse {
    return attendanceBonusInsightsResponseSchema.parse({
      jobId,
      jobTitle,
      annee: year,
      mois: month,
      ...
      source,
      reglePrime: AnalysisAiService.REGLE_PRIME_DT,
      backendMetrics: backendMetrics.map((m) => ({
        resourceId: m.resourceId,
        displayName: m.displayName,
        annee: m.annee,
        mois: m.mois,
        joursOuvrables: m.joursOuvrables,
        joursPresentsOuvrables: m.joursPresentsOuvrables,
        joursAbsentsPointesOuvrables: m.joursAbsentsPointesOuvrables,
        joursOuvrablesSansPointage: m.joursOuvrablesSansPointage,
        pointsMensuel: m.pointsMensuel,
        primeDt: m.primeDt,
        devise: 'DT' as const,
        joursPresent: m.joursPresent,
        joursAbsent: m.joursAbsent,
        totalJours: m.totalJours,
        tauxPresence: m.tauxPresence,
        heuresTotales: m.heuresTotales,
        heuresMoyennesJourPresent: m.heuresMoyennesJourPresent,
        scoreRendement: m.scoreRendement,
      })),
      analysis: {
        summary: analysis.summary,
        recommendationsEquipe: analysis.recommendationsEquipe.slice(0, 8),
        travailleurs: analysis.travailleurs,
        confiance: analysis.confiance,
      },
    });
  }

  private mergeAttendanceBonusWithBackend(
    parsed: AttendanceBonusAnalysisPayload,
    backendMetrics: WorkerMonthlyAttendanceMetrics[],
  ): AttendanceBonusAnalysisPayload {
    const byId = new Map(parsed.travailleurs.map((t) => [t.resourceId, t]));
    const travailleurs: TravailleurBonusRow[] = backendMetrics.map((m) => {
      const row = byId.get(m.resourceId);
      const base: TravailleurBonusRow = row
        ? {
            ...row,
            nomAffiche: row.nomAffiche?.trim() ? row.nomAffiche : m.displayName,
            scorePerformance: Math.min(30, Math.max(0, row.scorePerformance)),
          }
        : this.buildFallbackTravailleurRow(m);
      return this.applyPrimeBarème(base, m);
    });
    const recommendationsEquipe =
      parsed.recommendationsEquipe.length > 0
        ? parsed.recommendationsEquipe
        : this.buildFallbackRecommendations(backendMetrics);
    return {
      summary:
        parsed.summary.trim().length > 0
          ? parsed.summary
          : this.buildFallbackAttendanceSummary(backendMetrics),
      recommendationsEquipe,
      travailleurs,
      confiance: Math.min(1, Math.max(0, parsed.confiance)),
    };
  }

  private applyPrimeBarème(
    row: TravailleurBonusRow,
    m: WorkerMonthlyAttendanceMetrics,
  ): TravailleurBonusRow {
    return {
      ...row,
      scorePerformance: m.pointsMensuel,
      montantPrimeSuggere: m.primeDt,
      montantBonusSuggere: null,
      justification: this.primeJustification(m),
    };
  }

  private primeJustification(m: WorkerMonthlyAttendanceMetrics): string {
  ...
  private buildFallbackAttendanceAnalysis(
    backendMetrics: WorkerMonthlyAttendanceMetrics[],
  ): AttendanceBonusAnalysisPayload {
    return {
  ...
  private async callGroqJson(
    ...
    this.mapGroqErrors(res);
    const data = await this.parseGroqChatCompletionBody(res);
    this.logger.log('groq attendance json ok');
