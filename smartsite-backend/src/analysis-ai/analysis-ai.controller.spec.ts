...
const authReq = {
  user: { sub: '65cfa1a7cf3f4e38dc1db123', roleName: 'Admin' },
};

describe('AnalysisAiController', () => {
  const mockProjects = {
    assertRequestCanAccessProject: jest.fn().mockResolvedValue({}),
  };
  ...
  it('returns 200 payload from service', async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        { provide: AnalysisAiService, useValue: mockService },
    ...
    await expect(
      controller.analyzeProjectInsights(
        authReq,
        '507f1f77bcf86cd799439099',
        body,
      ),
    ).resolves.toEqual(analysisPayload);
    expect(mockService.generateInsights).toHaveBeenCalledWith(
      '507f1f77bcf86cd799439099',
    );
  });
  ...
  it('returns initial report from service', async () => {
    const mockService = {
      ...
      initialAssistantReport: jest
        .fn()
        .mockResolvedValue({ report: '## Projet\n…' }),
    };
    const module: TestingModule = await Test.createTestingModule({
      controllers: [AnalysisAiController],
      providers: [
    ...
    const controller = module.get(AnalysisAiController);
    await expect(
      controller.assistantInitialReport(
        authReq,
        '507f1f77bcf86cd799439099',
        new AssistantInitialReportDto(),
      ),
    ).resolves.toEqual({ report: '## Projet\n…' });
    expect(mockService.initialAssistantReport).toHaveBeenCalledWith(
      '507f1f77bcf86cd799439099',
    );
  });
  ...
  it('returns assistant reply from service', async () => {
    ...
    await expect(
      controller.assistantChat(authReq, '507f1f77bcf86cd799439099', dto),
    ).resolves.toEqual({
      reply: 'Voici une réponse.',
    });
    expect(mockService.chatProject).toHaveBeenCalledWith(
      '507f1f77bcf86cd799439099',
      dto,
    );
  });
