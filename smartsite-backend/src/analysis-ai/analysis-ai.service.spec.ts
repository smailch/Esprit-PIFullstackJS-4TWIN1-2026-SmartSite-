...
describe('AnalysisAiService', () => {
  ...
  const mockAttendance = { findByJob: jest.fn() };
  const mockJobs = { findOne: jest.fn() };
  const mockConfig = {
  ...
  beforeEach(async () => {
    ...
    mockJobs.findOne.mockResolvedValue({ title: 'Job test' });
    mockAttendance.findByJob.mockResolvedValue([]);
  });
