...
export class UsersService {
  ...
  /** Comptes avec le rôle « Client » (liste pour formulaire projet), sans mot de passe. */
  async findClientAccounts() {
    const role = await this.roleModel.findOne({ name: 'Client' }).lean();
    if (!role?._id) {
      return [];
    }
  ...
  async addUserProjectId(userId: string, projectId: string): Promise<void> {
  ...
  async removeUserProjectId(userId: string, projectId: string): Promise<void> {
    if (!Types.ObjectId.isValid(userId) || !Types.ObjectId.isValid(projectId)) {
      throw new BadRequestException('Invalid user or project id');
    }
    await this.userModel.findByIdAndUpdate(userId, {
      $pull: { projectIds: new Types.ObjectId(projectId) },
    });
  }
